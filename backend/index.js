  ...
  try {
    ...
    app.put("/loans/:id", verifyJWT, async (req, res) => {
      try {
        ...
        // Allow admin to edit any loan, manager to edit only their own loans
        if (user.role === "admin") {
          // Admin can edit any loan
        } else if (user.role === "manager") {
          // Manager can only edit their own loans
          const loan = await loanCollection.findOne({ _id: new ObjectId(id) });
          if (!loan) {
            return res.status(404).send({ message: "Loan not found" });
          }
          if (loan.manager?.email !== user.email) {
            return res
              .status(403)
              .send({ message: "You can only edit your own loans" });
          }
        } else {
          return res
            .status(403)
            .send({ message: "Admin or Manager access required" });
        }
    ...
    app.delete("/loans/:id", verifyJWT, async (req, res) => {
      ...
      try {
        ...
        // Allow admin to delete any loan, manager to delete only their own loans
        if (user.role === "admin") {
          // Admin can delete any loan
        } else if (user.role === "manager") {
          // Manager can only delete their own loans
          const loan = await loanCollection.findOne({ _id: new ObjectId(id) });
          if (!loan) {
          ...
          if (loan.manager?.email !== user.email) {
            return res
              .status(403)
              .json({ message: "You can only delete your own loans" });
          }
        } else {
          return res
            .status(403)
    ...
    app.get("/admin-stats", async (req, res) => {
      const totalLoans = await loanApplicationCollection.countDocuments();
      const approvedLoans = await loanApplicationCollection.countDocuments({
        status: "Approved",
      });
      const rejectedLoans = await loanApplicationCollection.countDocuments({
        status: "Rejected",
      });
      const pendingLoans = await loanApplicationCollection.countDocuments({
        status: "Pending",
      });
      const paidLoans = await loanApplicationCollection.countDocuments({
        applicationStatus: "Paid",
      });
